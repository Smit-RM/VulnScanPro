// ==================== STATE ====================
    ...
    function initSocket() {
      ...
      try {
        ...
        _socket.on('scan_progress', (data) => {
          ...
          if (phase) phase.textContent = data.message || '';
          if (log && data.message) {
          ...
          if (data.progress >= 100 || (data.message || '').toLowerCase().includes('cancel')) {
            _activeScanId = null;
            ...
            if ((data.message || '').toLowerCase().includes('complet')) {
              toast('Scan completed!', 'success');
            } else if ((data.message || '').toLowerCase().includes('cancel')) {
    ...
    async function _launchScanOnTarget(targetId, config, btn, cancelBtn) {
      const resp = await apiFetch(`${API_BASE}/api/scans`, {
        method: 'POST',
        body: JSON.stringify({ target_id: targetId, config })
      });
      if (!resp.ok) { const d = await resp.json(); toast('Scan failed: ' + (d.error||'unknown'), 'error'); _resetScanBtn(btn, cancelBtn); return; }
      const scan = await resp.json();
      _activeScanId = scan.id;
      state.scanRunning = true;
      btn.innerHTML = '<span class="spin"></span> Scanning...';
      initSocket();
      if (_socket) {
        _socket.emit('join_scan', { scan_id: scan.id });
      } else {
        // Fallback: poll scan status and progress
        const pollInterval = setInterval(async () => {
          if (!_activeScanId || _activeScanId !== scan.id) {
            clearInterval(pollInterval);
            return;
          }
          try {
            const pollResp = await apiFetch(`${API_BASE}/api/scans/${scan.id}`);
            if (!pollResp.ok) {
            ...
            const sData = await pollResp.json();
            ...
            let progressVal = sData.progress || 0;
            ...
            if (sData.status === 'completed' || sData.status === 'failed' || sData.status === 'cancelled') {
              ...
              if (sData.status === 'completed') {
                ...
                if (phase) phase.textContent = 'Completed';
                toast('Scan completed!', 'success');
              } else if (sData.status === 'cancelled') {
                if (phase) phase.textContent = 'Cancelled';
                toast('Scan cancelled.', 'info');
              } else {
            ...
            if (phase) phase.textContent = `Scanning... (${progressVal}%)`;
            
            // Poll vulnerabilities
            const vResp = await apiFetch(`${API_BASE}/api/scans/${scan.id}/vulnerabilities`);
            if (vResp.ok) {
              const vulns = await vResp.json();
              const r = document.getElementById('scan-results');
              if (r) {
                r.innerHTML = vulns.map(v => {
                  const sevClass = { critical:'bc', high:'bh', medium:'bm', low:'bl', info:'bi' }[v.severity] || 'bi';
                  return `<div class="badge-row" style="padding:8px 10px;background:rgba(255,255,255,.04);border-radius:8px;margin-bottom:6px;border:1px solid var(--border);display:flex;gap:8px;align-items:flex-start">
                    <span class="badge ${sevClass}" style="flex-shrink:0">${v.severity}</span>
    ...
    async function cancelCurrentScan() {
      ...
      try {
        await apiFetch(`${API_BASE}/api/scans/${_activeScanId}`, { method: 'DELETE' });
    ...
    async function nav(page) {
      ...
      if (page === 'scanner') { populateScanProjectDropdown(); loadSchedules(); }
  ...
  `;
    ...
    async function startScan() {
      if (state.scanRunning) { toast('Scan already running', 'error'); return; }
      const url = document.getElementById('surl').value.trim();
      const zapUrl = document.getElementById('zap-url') ? document.getElementById('zap-url').value.trim() : '';
      const zapKey = document.getElementById('zap-key') ? document.getElementById('zap-key').value.trim() : '';
      
      if (!url) { toast('Enter a target URL', 'error'); return; }
      if (!zapUrl) { toast('Enter ZAP API URL', 'error'); return; }
      
      try { new URL(url); new URL(zapUrl); } catch { toast('Invalid URL format', 'error'); return; }

      state.scanRunning = true;
      const btn = document.getElementById('scan-btn');
      btn.disabled = true;
      btn.innerHTML = '<span class="spin"></span> Fetching from ZAP...';
      ...
      pct.textContent = '50%';
      phase.textContent = 'Connecting to ZAP API...';
      
      function addLog(msg) {
        const line = document.createElement('div');
        line.style.cssText = 'padding:2px 0;border-bottom:1px solid rgba(255,255,255,.04)';
        line.innerHTML = `<span style="color:var(--text3);margin-right:8px">[${new Date().toTimeString().slice(0, 8)}]</span>${msg}`;
        log.appendChild(line);
        log.scrollTop = log.scrollHeight;
      }

      addLog('Fetching alerts from ' + zapUrl + ' (via JSONP)');

      try {
        const apiUrl = `${zapUrl}/JSONP/core/view/alerts/?apikey=${encodeURIComponent(zapKey)}`;
        
        // Custom JSONP fetch to bypass CORS
        const data = await new Promise((resolve, reject) => {
          const callbackName = 'zapJsonpResult';
          
          // Save the old one if it exists, just in case
          const oldCallback = window[callbackName];
          
          window[callbackName] = function(response) {
            window[callbackName] = oldCallback; // Restore
            document.body.removeChild(script);
            resolve(response);
          };
          
          const script = document.createElement('script');
          script.src = apiUrl; // ZAP ignores callback parameter and always uses zapJsonpResult
        ...
        // Filter alerts intelligently by hostname to avoid strict trailing-slash mismatches
        const targetHostname = new URL(url).hostname;
        let alerts = allAlerts.filter(a => {
          try {
            return new URL(a.url).hostname === targetHostname;
          } catch(e) {
            return false;
          }
        });
        ...
        addLog('Successfully retrieved ' + alerts.length + ' alerts for ' + targetHostname);

        const scanId = 'sc' + Date.now();
        
        // Map ZAP alerts to app format
        const findings = alerts.map(a => {
          let sev = 'info';
          if (a.riskcode === '3') sev = 'critical';
          else if (a.riskcode === '2') sev = 'high';
          else if (a.riskcode === '1') sev = 'medium';
          else if (a.riskcode === '0') sev = 'low';
          else if (a.risk === 'High') sev = 'high';
          else if (a.risk === 'Medium') sev = 'medium';
          else if (a.risk === 'Low') sev = 'low';
          else if (a.risk === 'Informational') sev = 'info';

          return {
            title: a.alert || 'Unknown Alert',
            severity: sev,
            desc: a.description || 'No description provided by ZAP.',
            url: a.url || url,
            check: 'zap',
            cwe: a.cweid && a.cweid !== '-1' ? 'CWE-' + a.cweid : ''
          };
        });

        // Clear out old data and fake demo data so we ONLY show real ZAP findings
        state.scans = [];
        state.vulns = [];

        state.scans.push({ id: scanId, url, profile: 'zap-import', status: 'completed', ts: Date.now(), vulnCount: findings.filter(f => f.severity !== 'info').length });
        findings.forEach(f => state.vulns.push({ ...f, id: 'v' + Math.random().toString(36).slice(2), scanId, ts: Date.now() }));
        save();

        results.innerHTML = `<div style="padding:12px;background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.2);border-radius:9px">
          <div style="font-weight:700;color:var(--green);margin-bottom:6px">✅ Import completed — ${findings.length} findings</div>
          ${['critical', 'high', 'medium', 'low', 'info'].map(s => {
              const n = findings.filter(f => f.severity === s).length;
              return n ? `<span class="badge b${s.charAt(0)}" style="margin-right:5px">${s}: ${n}</span>` : '';
            }).join('')}
          <div style="margin-top:8px"><button class="btn btn-p btn-sm" onclick="currentScanId='${scanId}';nav('vulns')">View Vulnerabilities →</button></div>
        </div>`;
        toast('Scan results imported!', 'success');

      } catch (err) {
        addLog('<span style="color:#ef4444">Error connecting to ZAP API: ' + err.message + '</span>');
        const testUrl = `${zapUrl}/JSON/core/view/alerts/?apikey=${encodeURIComponent(zapKey)}&baseurl=${encodeURIComponent(url)}`;
        addLog(`<strong><a href="${testUrl}" target="_blank" style="color:var(--cyan);text-decoration:underline;font-size:14px;display:block;margin-top:4px">👉 CLICK HERE to open ZAP in a new tab and see the exact error message!</a></strong>`);
        toast('ZAP connection failed', 'error');
        ...
        state.scanRunning = false;
        btn.disabled = false;
        btn.innerHTML = '🚀 Fetch Results From ZAP';
      }
    }
    ...
    function renderVulns() {
      ...
      if (currentScanId) {
        ...
        bannerEl.innerHTML = `<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.25);border-radius:9px;margin-bottom:10px;font-size:12px">
          ...
          <button class="btn btn-g btn-xs" onclick="currentScanId=null;renderVulns()">Show All Scans ✕</button>
        </div>`;
  ...
  <div style="font-size:11px;color:var(--text3);padding:8px 12px;border-top:1px solid var(--border)">${list.length} issue${list.length !== 1 ? 's' : ''} · click any row to view attack details</div>`;
    ...
    function showVulnDetail(id) {
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:4px">
        ...
        <button class="btn btn-g btn-xs" onclick="document.getElementById('vuln-detail-panel').style.display='none';document.getElementById('vulns-layout').style.gridTemplateColumns='1fr';selectedVulnId=null;renderVulns()">✕</button>
      </div>
      <div class="vd-header">
        <span class="vd-sev-badge" style="background:${sevBg[v.severity]};color:${sevClr[v.severity]}">${v.severity.toUpperCase()}</span>
        <div>
